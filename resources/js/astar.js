...
window.showRoute = async function(lat, long, idMosque) {
    navigator.geolocation.getCurrentPosition(async position => {
        ...
        const node_dari = Astar.Node('dari', LOKASI_SEKARANG.lat, LOKASI_SEKARANG.lng);
        const node_tujuan = Astar.Node('tujuan', lokasi_tujuan.lat, lokasi_tujuan.lng);

        let result = astar_find(all_edges, node_dari, node_tujuan, (result, summary) => {
 ...
 * @returns {*}
 */
const astar_find = async (all_edges, dari, tujuan, result_callback) => {
    const t0 = performance.now();
    const list = (node, next) => {
        const _edges = all_edges.filter(edge => edge.from === node.id);
        const _results = [];
    ...
    const astar_run = () => new Promise((resolve, reject) => {
        astar.search(dari, tujuan, function(err, result) {
            resolve(result);
        });
    });

    const path = await astar_run();

    const t1 = performance.now();
    const summary = {
        time_ms: t1 - t0, total_node: all_edges.length
    };
...
const draw_way_to_map = function(route, edges, color) {
    ...
    for (let i = 0; i < route.length; i++) {
        const node = route[i];
        const pp = edges.find(element => element.nama === node.id);
        let { lat, lng } = pp;
        _waypoints.push(L.latLng(lat, lng))
    }
    let control = L.Routing.control({
